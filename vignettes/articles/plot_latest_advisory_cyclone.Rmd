---
title: "Plot Current and Forecast Positions of a Cyclone"
author: "Tim Trice"
date: "June 14, 2017"
...
load_storm_data(years = 2008, basins = "AL", products = "fstadv")
```

Filter out Hurricane Ike data.

```{r}
fstadv <- AL2008_fstadv %>% filter(Key == key, Adv <= adv)
```
...
Tidy advisory points (removing forecast and wind radius details).

```{r}
df_tidy_adv <- tidy_fstadv(fstadv)
```

Tidy forecast points.
...
Plot to-date track and forecast.
...
Rebuild a base plot of the Atlantic ocean.

```{r}
bp <- al_tracking_chart(color = "black", fill = "white", size = 0.1, res = 50)
```

We can "zoom in" on the chart with `bbox`.
