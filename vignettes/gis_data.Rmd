...
```{r}
fcst_line <- shp_to_df(adv$al182012_018_5day_lin)
```
...
```{r}
(bb <- sp::bbox(adv$al182012_018_5day_lin))
```
...
Each forecast position is also included in the points dataframe. You can access this in the `adv$al182012_018_5day_pts@data` object; no conversion necessary.

```{r, fig.width = 7, fig.align = "center"}
(p3 <- p2 + geom_point(data = adv$al182012_018_5day_pts@data, aes(LON, LAT)))
```
...
```{r}
fcst_cone <- adv$al182012_018_5day_pgn
...
```{r}
ww_line <- shp_to_df(adv$al182012_018_ww_wwlin)
```
...
```{r}
prob_surge <- shp_to_df(ss$al142016_2016100618_gt0)
bb <- sp::bbox(ss$al142016_2016100618_gt0)
```
...
```{r}
wf_init <- shp_to_df(wf$al142016_2016100606_initialradii)
bb <- sp::bbox(wf$al142016_2016100606_forecastradii)
```
...
```{r}
wf_fcst <- shp_to_df(wf$al142016_2016100606_forecastradii)
```
...
```{r, fig.width = 7, fig.align = "center"}
bb <- sp::bbox(wsp$`2016100606_wsp34knt120hr_halfDeg`)
(p5 <- p + 
     geom_point(data = broom::tidy(wsp$`2016100606_wsp34knt120hr_halfDeg`), 
...
```{r, fig.width = 7, fig.align = "center"}
bb <- sp::bbox(wsp$`2016100606_wsp50knt120hr_halfDeg`)
(p6 <- p + 
     geom_point(data = broom::tidy(wsp$`2016100606_wsp50knt120hr_halfDeg`), 
                aes(x = coords.x1, y = coords.x2, color = PWIND120))) + 
    coord_equal(xlim = c(bb[1,1], bb[1,2]), ylim = c(bb[2,1], bb[2,2]))
```
...
```{r, fig.width = 7, fig.align = "center"}
bb <- sp::bbox(wsp$`2016100606_wsp64knt120hr_halfDeg`)
(p7 <- p + 
     geom_point(data = broom::tidy(wsp$`2016100606_wsp64knt120hr_halfDeg`), 
                aes(x = coords.x1, y = coords.x2, color = PWIND120))) + 
    coord_equal(xlim = c(bb[1,1], bb[1,2]), ylim = c(bb[2,1], bb[2,2]))
```
