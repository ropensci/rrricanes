...
```{r}
str(df.gis_adv$al182012_018_5day_lin)
```
...
```{r}
fcst_line <- shp_to_df(df.gis_adv$al182012_018_5day_lin)
```
...
```{r}
(bb <- sp::bbox(df.gis_adv$al182012_018_5day_lin))
```
...
Each forecast position is also included in the points dataframe. You can access this in the `df.gis_adv$al182012_018_5day_pts@data` object; no conversion necessary.

```{r, fig.width = 7, fig.align = "center"}
(p3 <- p2 + geom_point(data = df.gis_adv$al182012_018_5day_pts@data, aes(LON, LAT)))
```
...
```{r}
fcst_cone <- df.gis_adv$al182012_018_5day_pgn
...
```{r}
ww_line <- shp_to_df(df.gis_adv$al182012_018_ww_wwlin)
```
...
(p5 <- p + 
     geom_point(data = broom::tidy(df.gis_wsp$`2016100606_wsp34knt120hr_halfDeg`), 
...
(p6 <- p + 
     geom_point(data = broom::tidy(df.gis_wsp$`2016100606_wsp50knt120hr_halfDeg`), 
                aes(x = coords.x1, y = coords.x2, color = PWIND120))) + 
    coord_equal(xlim = c(bb[1,1], bb[1,2]), ylim = c(bb[2,1], bb[2,2]))
```
...
(p7 <- p + 
     geom_point(data = broom::tidy(df.gis_wsp$`2016100606_wsp64knt120hr_halfDeg`), 
                aes(x = coords.x1, y = coords.x2, color = PWIND120))) + 
    coord_equal(xlim = c(bb[1,1], bb[1,2]), ylim = c(bb[2,1], bb[2,2]))
```
